...
public class MainClassUseXmlsImport {
	...
	public static void main(String [] args) {
		...
		//학생 한명 마다 객체화 시키기 위한 assembler - 변수가 아닌 객체를 set,get하는 DTO 같음, java프로그래밍에서 의존주입을 위해.
		...
		//설정파일을 import로 처리한 경우 appCtxImport.xml 하나만.
		GenericXmlApplicationContext ctx =
		...
		for(int j = 0; j < sNums.length; j++) {
			Student student = new Student(sNums[j], sIds[j], sPws[j], 
					sNames[j], sAges[j], sGenders[j], sMajors[j]); //학생들을 가져와서
			registerService.register(student); //등록
		}
